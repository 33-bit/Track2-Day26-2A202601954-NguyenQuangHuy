...
def test_starter_end_to_end_against_the_full_fixture_set(labelled_fixtures):
    ...
    assert report["rejected"] == 0, "must not emit invalid claims"

    assert report["precision"] == 1.0
    assert report["recall"] == 1.0, f"expected perfect recall 1.0, got {report['recall']}"
    assert report["false_claim_rate"] == 0.0

    for cls in CLASSES:
        assert report["per_class"][cls]["recall"] == 1.0, f"{cls} not perfect"
